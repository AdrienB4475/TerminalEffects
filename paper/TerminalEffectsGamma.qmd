...
ggplot(lac_pred_pop, aes(x = age, y = predicted)) +
  ...
  geom_line(aes(
    ...
    geom_ribbon(aes(ymin = conf_lo, ymax = conf_hi, fill = ageclass), alpha = 0.2) +  
...
wt_mod2 <- lmer(Wt ~ age10 * ageclass + (1 | year) + (1 | animalID), 
     data = wt_dat,
     )

wt_mod_um <- as_lmerModLmerTest(wt_mod)
summary(wt_mod)
...
#If not this ^^ maybe we just mimic the effects we want by creating a threshold ourselves? 
wt_dat$age10_thr1 <- pmax(0, wt_dat$age10 - 6)  # First breakpoint at age 6
wt_dat$age10_thr2 <- pmax(0, wt_dat$age10 - 11) # Second breakpoint at age 11

wt_mod <- lmer(Wt ~ age10 + age10_thr1 + age10_thr2 + 
               (1 | year) + (1 | animalID), 
               data = wt_dat)
...
wt_grid <- expand_grid(
  ...
  mutate(age10 = (age - maturity) / 10, 
         ageclass = ifelse(age <= maturity, "young", "old"),
         age10_thr1 = pmax(0, age10 - 6),  # Ensure these match wt_dat
         age10_thr2 = pmax(0, age10 - 11))
