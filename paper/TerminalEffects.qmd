...
ggplot(wt_dat, aes(x = age, y = Wt)) +
  ...
  scale_color_manual(values = c("#D5A6F6", "#66B2B2"))  
...
wt_dat <- wt_dat %>%
  mutate(
    ...
    ageclass3 = factor(ageclass3, levels = c("young", "prime", "old")),
    age10 = (age - maturity) / 10, 
    year_fct = factor(year)
  )
...
#   ungroup()
# 
# #my stomach hurts. please work:
# surv_dat <- surv_seal %>%
#   dplyr::select(animalID, pupID, bio_age, year, pupsex, age) %>%  
#   mutate(
#     is_survive = as.integer(pupID %in% animalID),  # Did the pup survive? 1 if yes, 0 if no
#     terminal = ifelse(bio_age == 0, 1, 0),         
#     ageclass = ifelse(age <= maturity, "young", "old"),  
#     age10 = (age - maturity) / 10,                
#     year_fct = factor(year)                       
#   )
# 
# #first lets make yet another new dataframe 
# ofsp_surv <- surv_dat
# 
# #Did seal survive??
# ofsp_surv$is_survive <- as.integer(ofsp_surv$pupID %in% ofsp_surv$animalID)
# 
# #clean our data again then: 
# surv_dat <- ofsp_surv %>%
#   dplyr::select(animalID, pupID, bio_age, year, pupsex, age, is_survive) %>%
#   filter(!is.na(is_survive)) %>%
#   mutate(
#     terminal = ifelse(bio_age == 0, 1, 0),
#     ageclass = ifelse(age <= maturity, "young", "old"),
#     age10    = (age - maturity) / 10,
...
#   )
# 
# #Model time using glmer: i actuallt give up i think?? im going um INSANE. goodnight
# surv_mod <- glmer(
#   is_survive ~ terminal * ageclass + (1 | animalID), 
#   family = binomial,
#   data = surv_dat,
...
#   age = 3:22,  # range of ages for prediction
#   terminal = 0:1  # terminal status (0 = nonterminal, 1 = terminal)
# ) %>%
#   mutate(
#     age10 = (age - maturity) / 10,  # scale age
#     ageclass = ifelse(age <= maturity, "young", "old")
#   )
# 
# #And then ofc we make our pred after the grid right??
...
#   )
# 
# #Heres where everything falls apart ofc.. or not??
# surv_summ <- surv_dat %>%
#   ungroup() %>%
#   mutate(terminal_fct = factor(terminal, labels = c("Nonterminal", "Terminal"))) %>%
#   group_by(age, terminal_fct, ageclass) %>%
#   summarize(
#     prop_surv = mean(is_survive, na.rm = TRUE),
...
# 
# 
# #anddd.. something off cause no way pup survival that low for both.. UGHHHH 
# 
# 
# 
# #i wonder if its the kind of glmer model.. new???
# surv_mod_quasi <- glmer(
#   is_survive ~ terminal * ageclass + (1 | year_fct) + (1 | animalID),
#   family = binomial(link = "logit"),
#   data = surv_dat
# )
# summary(surv_mod_quasi)
# 
# surv_pred_pop_quasi <- surv_grid %>%
#   mutate(
#     predicted_prob_survival = predict(
#       surv_mod_quasi, 
#       newdata = surv_grid, 
...
#   )
# 
# #Quasi summary incoming:
# surv_summ_quasi <- surv_dat %>%
#   ungroup() %>%
#   mutate(terminal_fct = factor(terminal, labels = c("Nonterminal", "Terminal"))) %>%
#   group_by(age, terminal_fct, ageclass) %>%
#   summarize(
...
# 
# #New plot??
# ggplot(surv_pred_pop_quasi, aes(x = age, y = predicted_prob_survival)) +
#   geom_pointrange(aes(
#     y = prop_surv, 
#     ymin = prop_surv - 1.96 * se_prop, 
#     ymax = prop_surv + 1.96 * se_prop, 
