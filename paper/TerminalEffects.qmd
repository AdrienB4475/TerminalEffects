...
lact_dat <- bioseal %>% 
  ...
  mutate(terminal = ifelse(bio_age == 0, 1, 0), 
         ageclass = ifelse(age >= maturity, "old", "young"), 
...
lac_mod <- glmer.nb(lactation_duration ~ age10 + terminal:ageclass + (1 | year),
         data = lact_dat)
...
lac_grid <- expand_grid(
  ...
  mutate(age10 = (age - maturity) / 10, 
         ageclass = factor(ifelse(age >= maturity, "old", "young"), 
                           levels = c("old", "young")))
...
lact_plot <- ggplot(lac_pred_pop, aes(x = age, y = predicted)) +
  geom_point(data = lact_dat, aes(x = age, y = lactation_duration), alpha = 0.5, color = "gray") +  # Raw data points
  geom_line(aes(group = interaction(ageclass, year_fct)), alpha = 0.1, data = lac_pred_pop) +  # Predicted lines by age class and year
  ...
  geom_line(aes(y = predicted, group = ageclass), linetype = "dotted", linewidth = 1.2, alpha = 0.5) +  # Dotted predicted lines
