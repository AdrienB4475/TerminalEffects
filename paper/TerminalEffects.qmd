...
lact_dat <- bioseal %>% 
  ...
  mutate(year_fct = factor(year), 
         lac_rescale = (lactation_duration / 12)) %>% 
           select(animalID, 
                  year_fct, 
                  bio_age, 
                  lactation_duration,
...
# lac_sd <- sqrt((sum((lact_dat$lactation_duration - mean(lact_dat$lactation_duration))^2)) / length(lact_dat$lactation_duration))
...
lact_mod <- lmer(
  lactation_duration ~ bio_age + (1 | animalID),  # Fixed and random effects
  data = lact_dat
)
summary(lact_mod)
...
ggplot(lact_dat, aes(x = bio_age, y = lactation_duration)) +
  ...
  geom_line(aes(y = predicted, group = animalID), color = "blue") +  # Predicted values
  theme_minimal()
```
